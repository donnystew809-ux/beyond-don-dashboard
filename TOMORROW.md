...
> Status as of tonight: the dashboard is **built, deployed locally, and already
> showing your real data**. 11 properties, 27 reservations, 2,164 daily price
> rows synced from PriceLabs. Code pushed to GitHub. Two things still need
> your fingers to finish: a few clicks to deploy to Vercel, and pasting in
> Airbnb iCal URLs (only Airbnb makes you log in, so I couldn't grab them
> myself).
...
npm run dev
```
Open http://localhost:3000, enter `beyonddonllc@outlook.com`, click the magic
link Supabase sends, and you should land on the portfolio dashboard with
real numbers. Click around — Calendar, Pricing, Properties — all populated
from PriceLabs.
...
### 3. Deploy to Vercel (so it runs without your laptop)
1. Go to https://vercel.com — sign in with GitHub (one click)
2. Click **Add New → Project**
3. Pick **donnystew809-ux/beyond-don-dashboard** from the list
4. Under **Environment Variables**, paste in everything from your local
   `.env.local` (open the file in any editor, copy-paste each line)
5. Click **Deploy**

Vercel picks up `vercel.json` automatically and runs the cron jobs:
- iCal sync every 2 hours
- Turno sync every hour
- PriceLabs sync every 6 hours

(Optional later: add a custom domain like `dash.beyonddon.com` in Vercel
project settings.)
